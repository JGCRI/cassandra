...
def init_rgn_tables(rgnconfig):
    ...
    (_, _regions_ordered) = util.rd_rgn_table('%s/RgnNames.txt' %
                                              rgnconfig)  # Variable being ignored is reverse lookup table
...
def proc_wdlivestock(infilename, outfilename, rgnTotalFilename):
    ...
    for region in _regions_ordered:
        ...
        try:
            total_wd[region] = map(lambda tb, sh, pt, pg: tb+sh+pt+pg, total_bovine,
                                   sheepgoat, poultry[region], pig[region])
        except TypeError:
