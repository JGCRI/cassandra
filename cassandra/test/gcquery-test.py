...
class testQueries(unittest.TestCase):
    def setUp(self):
        ...
        self.global_params = GlobalParamsComponent({})
    ...
    def testInit_rgn_tables(self):
        ...
        of the correct type."""
        # set up the region tables for the waterdisag component
        ...
        for table in (waterdisag._gis2000, waterdisag._bfracFAO2005, waterdisag._gfracFAO2005, waterdisag._gcam_rgn_translation):
    ...
    def testProc_wdnonag(self):
        ...
        # For some reason China is excluded from queries; normal?
        self.assertEqual(len(wd_elec), self.tot_regions - self.num_drops - 1)
    ...
    def testProc_wdlivestock(self):
        ...
        wd_liv = waterdisag.proc_wdlivestock('./sample-out/batch-water-livestock.csv',
                                             './sample-out/final_wd_liv.csv', './sample-out/rgn_tot_withd_liv.csv')

        self.assertIs(type(wd_liv), dict)
        # 5 types of livestock in 31 regions (no Taiwan), minus header
        self.assertEqual(len(wd_liv), (self.gcam_regions-1)*5 - 1)
